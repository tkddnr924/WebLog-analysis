...
| 연산 | `== != > >= < <=` · `in 400..499`(범위) · `in ("10.0.0.1", "10.0.0.2")`(목록) · `contains` `icontains` `startswith` `endswith` `matches` · `is null` |
| 조합 | `and` `or` `not` · `any of them` · `all of ($a*)` |

찾을 값이 여럿이면 `or`를 늘어놓는 대신 목록으로 쓴다. `ip in ("10.0.0.1", "10.0.0.2", "203.0.113.7")`, `status in (404, 500)`처럼 한 줄이면 된다.

내장 룰 27개로 바로 시작한다. 접근 로그는 **웹 페이지 요청만**(js·css·이미지·폰트 같은 정적 리소스를 걷어낸다), SQL Injection, XSS, 경로 탐색, 명령 주입, 스캐너 경로, sqlmap, 스캐너 UA, Log4Shell, 파일 포함·SSRF, 웹셸 업로드·실행, 봇 UA 등 15개. 에러 로그는 심각·오류·경고, 파일 없음, 권한 거부, 업스트림 연결 실패, 요청 본문 초과, SSL 핸드쉐이크 오류, PHP·FastCGI 실패 등 12개. 원문을 열어 고치면 자기 룰이 되고, 편집기가 문법 오류 위치를 실시간으로 짚는다. 만든 룰은 편집기의 **룰 삭제** 또는 사이드바의 `−`로 지운다.

사이드바 맨 위의 **기간**과 **화이트리스트 IP**는 룰보다 상위 조건이다. 사고 시각을 넣으면 어느 화면으로 옮겨도 그 구간만 보고, 화이트리스트에 넣은 주소(`10.0.0.1` 또는 `1.1.*`)는 어떤 룰을 골라도 결과에서 빠진다. 분석가나 모니터링 장비의 요청을 걷어낼 때 쓴다.

## 결과를 다루는 방법

|  |  |
|---|---|
| **조회** | 수백만 행을 스크롤로 훑는다. 경로·IP·리퍼러·UA 통합 검색("초기화"로 검색어만 되돌린다), 행을 고르면 구조화 필드와 재구성한 로그 한 줄 |
| **통계** | 시간별 요청 수, 상태코드·메서드 분포, 상위 IP 표(번호·최초·마지막 탐지, **전체 CSV 저장**)·요청 대상. 에러는 레벨 분포와 상위 메시지 |
