...
내장 룰 27개로 바로 시작한다. 접근 로그는 SQL Injection, XSS, 경로 탐색, 명령 주입, 스캐너 경로, sqlmap, 스캐너 UA, Log4Shell, 파일 포함·SSRF, 웹셸 업로드·실행, 봇 UA 등 15개. 에러 로그는 심각·오류·경고, 파일 없음, 권한 거부, 업스트림 연결 실패, 요청 본문 초과, SSL 핸드쉐이크 오류, PHP·FastCGI 실패 등 12개. 원문을 열어 고치면 자기 룰이 되고, 편집기가 문법 오류 위치를 실시간으로 짚는다. 만든 룰은 편집기의 **룰 삭제** 또는 사이드바의 `−`로 지운다.
