...
3. 결과: 화면 맨 위 전체 너비에 `접근 로그` / `에러 로그` 탭이 있고(서류철 탭 모양, 활성 탭만 종이 면), 그 아래에 왼쪽 "룰" 사이드바와 오른쪽 화면이 딸린다. 오른쪽 위 작은 분절 스위치로 `조회`·`통계`를 고른다. 룰 목록은 종류별로 다르며(접근 룰은 상태·메서드·경로·UA, 에러 룰은 레벨·메시지·IP) 사용자 룰은 저장할 때의 종류에 붙는다. 룰은 YARA풍 텍스트로 쓴다(`src/lib/yara.ts` 파서, 테스트 있음).
   ```
   ...
   - 기간(사이드바 `[룰]` 아래): 시작·끝(제외)을 한국 시간으로 넣고 "기간 적용"을 누르면 **룰보다 상위 조건**으로 걸린다. 조회·통계, 접근·에러 어느 화면으로 옮겨도 유지되므로 사고 시각이 특정되면 그 구간만 보게 된다. "해제"로 전체 기간으로 돌아간다. 끝이 시작보다 앞이면 거부한다.
   - 접근 로그 · 조회: 기간 + 룰 조건 + 빠른 검색(경로·IP·리퍼러·UA)·정렬. 가상 스크롤 표, 커서 기반 추가 로드, UI 캐시 8MiB 상한. 행을 고르면 구조화 필드와 "재구성 로그"를 오른쪽에 보여준다. 현재 조건은 룰로 저장하거나 CSV/JSON Lines로 스트리밍 내보낼 수 있다(백그라운드, 취소 가능).
   - 접근 로그 · 통계: 시간별 요청 수, 상태코드·메서드 분포, 상위 N IP(최초·마지막 탐지 포함)·요청 대상.
   - 에러 로그 · 조회: 컬럼이 시간·레벨·클라이언트·메시지·출처다. 레벨과 메시지는 파싱 정의가 `extra_json`에 남긴 값에서 꺼내며 서버마다 키가 달라도(`level`, `client`, `error_code`, `message`) 화면에서 흡수한다. 기간 + 룰 조건 + 검색·정렬을 쓰고, 검색어는 메시지(대소문자 무시)나 클라이언트 IP 중 하나만 맞아도 걸린다.
   ...
   - 북마크: 별은 파일·줄로만 저장하므로 두 종류가 같은 표를 쓴다. 사이드바 맨 위의 "북마크" 룰은 로그 종류를 가리지 않고 접근·에러의 북마크를 한 목록에서 보여준다. 두 종류는 컬럼이 다르므로 이 목록은 시간과 한 줄 요약(접근은 `메서드 대상 · 상태 · IP`, 에러는 `레벨 · 메시지 · IP`)만 쓴다. 행을 고르면 상세는 그대로 열리고, 별을 다시 누르면 해제된다.
