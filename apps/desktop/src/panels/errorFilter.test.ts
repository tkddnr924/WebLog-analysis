...
import { openRange } from "../lib/timeRange";
...
describe("composeErrorFilter", () => {
  it("always pins the error kind and keeps the rule's own conditions", () => {
    ...
    const f = composeErrorFilter(base, form(), openRange);
  ...
  it("matches the search text against the message or the client ip", () => {
    expect(composeErrorFilter(emptyFilter(), form({ q: "  No such file  " }), openRange).expr).toEqual(search("No such file"));
    // 주소를 넣어도 같은 식이므로 IPv4·IPv6 구분 없이 클라이언트로 찾는다.
    expect(composeErrorFilter(emptyFilter(), form({ q: "2001:db8::1" }), openRange).expr).toEqual(search("2001:db8::1"));
  });

  it("ands the search with the rule condition", () => {
    const rule: FilterExpr = { kind: "cond", field: "level", op: "eq", value: "error" };
    const f = composeErrorFilter({ ...emptyFilter(), expr: rule }, form({ q: "refused" }), openRange);
  ...
  it("keeps the sidebar range above the rule's own range", () => {
    ...
    const f = composeErrorFilter(base, form({ q: "refused" }), { from: 1_704_038_400_000_000, to: null });
