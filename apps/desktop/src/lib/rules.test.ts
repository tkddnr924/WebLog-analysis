...
describe("builtin rules", () => {
  it("compile and carry names", () => {
    expect(BUILTIN_RULES.length).toBeGreaterThanOrEqual(19);
    expect(BUILTIN_RULES[0]).toMatchObject({ id: "builtin:all_requests", name: "전체 요청", builtin: true });
  ...
  it("signatures hit typical payloads and skip plain paths", () => {
    ...
    expect(evalExpr(byId("login_bruteforce_targets").expr, { ...req("/wp-login.php"), method: "POST" })).toBe(true);
    expect(evalExpr(byId("login_bruteforce_targets").expr, req("/wp-login.php"))).toBe(false);
    ...
    for (const r of BUILTIN_RULES.filter((x) => !["all_requests", "ok_responses"].some((id) => x.id === `builtin:${id}`))) {
